...
- na sequência o sistema encaminha o usuário para a tela de recorrências onde já verá as recorrências importadas dos cartões e será orientado a informar as demais recorrências que desejar, inclusive salários e outras receitas recorrentes informando ao usuário que ele pode informar uma valor aproximado e ajustar na hora de confirmar a recorrência como conta a pagar. As recorrências podem ser diárias, semanais, mensais, ou anuais. Recorrências não confirmadas e Margem Orçamentária são "previsão" uma recorrência "confirmada" é um compromisso assumido e gera lançamento contábil assim como as contas a pagar e a receber.
- compromisso (commitment) deve sempre ser criado com contrapartida associada (`counterpartyId` obrigatório), sem exceção no fluxo padrão.
- no fluxo de compromissos, a entrada primária deve representar o fato gerador (`BusinessTransaction`) e não o commitment direto: ao confirmar, o sistema reconhece contabilmente e gera os commitments derivados quando existirem.
- a tela principal de entrada operacional deve ser "Transação Financeira", centrada em fato gerador (`BusinessTransaction`) e não em cadastro direto de commitment.
- operação à vista pode não gerar commitment em aberto; parcelamento gera múltiplos commitments irmãos; recorrência continua fluxo separado.
- na entrada operacional, o usuário informa descrição, valor, contrapartida e forma de pagamento; o sistema deriva reconhecimento, liquidação imediata (quando à vista) e commitments/faturas conforme regras.
- no cartão de crédito, a compra individual gera reconhecimento e compõe faturas futuras; a obrigação aberta relevante para caixa deve estar no nível da fatura, evitando dupla contagem compra x fatura.
- no cartão de crédito, a obrigação financeira relevante nasce na fatura e pode ser ajustada por conciliação antes da liquidação.
- status atual de implementação de recorrências: suporte inicial apenas para periodicidade mensal.
...
- status atual da projeção: já há resumo textual de saldo projetado de disponibilidades (saldo base, entradas, saídas e saldo final projetado) para a janela do mês atual + 3 meses.

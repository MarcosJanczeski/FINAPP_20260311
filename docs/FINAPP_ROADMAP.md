# FINAPP Roadmap

## Fase 1 — Consolidação MVP
- [ ] Extrato operacional
- [ ] Ajustes de projeção (cartão)
- [ ] Refinos de UX restantes
- [ ] Commitments: consolidar naming/documentação para entrada por `BusinessTransaction`
- [ ] Commitments: introduzir entidade e contratos de `BusinessTransaction` com rastreabilidade mínima
- [ ] Commitments: criar use case de registro da transação (fato gerador)
- [ ] Commitments: orquestrar reconhecimento contábil + geração de commitments derivados
- [ ] Commitments UI: migrar tela atual para fluxo por fato gerador
- [ ] Projection: revisar impactos após migração do fluxo de origem

## Fase 2 — Refinamento pré-produção
- [ ] Separar telas sobrecarregadas
- [ ] Design system mínimo
- [ ] Estados da UI

## Fase 3 — Arquitetura
- [ ] Migrar para Supabase
- [ ] Multiusuário
- [ ] Segurança

## Fase 4 — Inteligência
- [ ] Projeção avançada
- [ ] Insights
